...
func TestClient_Install_NonRoot(t *testing.T) {
	...
	assert.Equal(t, []string{"dnf", "install", "-y", "curl", "wget"}, capturedArgs)
}
...
func TestClient_Install_Root(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "dnf", capturedName)
...
func TestClient_Update(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "sudo", capturedName)
	assert.Equal(t, []string{"dnf", "makecache"}, capturedArgs)
}
...
func TestClient_Upgrade(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "sudo", capturedName)
	assert.Equal(t, []string{"dnf", "upgrade", "-y"}, capturedArgs)
}
...
func TestClient_Upgrade_Specific(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "sudo", capturedName)
	assert.Equal(t, []string{"dnf", "upgrade", "-y", "curl", "wget"}, capturedArgs)
}
...
func TestClient_List(t *testing.T) {
	...
	assert.Len(t, results, 2)
	assert.Equal(t, "curl", results[0].Name)
	assert.Equal(t, "7.76.1-26.el9", results[0].Version)
	assert.Equal(t, "wget", results[1].Name)
	assert.Equal(t, "1.21.1-7.el9", results[1].Version)
}
...
func TestParseDnfList_Empty(t *testing.T) {
	results := parseDnfList("")
	assert.Empty(t, results)
}
...
func TestClient_Remove_NonRoot(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "sudo", capturedName)
	assert.Equal(t, []string{"dnf", "remove", "-y", "curl", "wget"}, capturedArgs)
}
