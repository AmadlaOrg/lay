...
func TestClient_Install(t *testing.T) {
	...
	assert.Equal(t, "choco", capturedName)
	assert.Equal(t, []string{"install", "-y", "curl", "wget"}, capturedArgs)
}
...
func TestParseChocoSearch(t *testing.T) {
	output := "curl 8.4.0\nwget 1.21\n2 packages found.\n"
	results := parseChocoSearch(output)
	assert.Len(t, results, 2)
	assert.Equal(t, "curl", results[0].Name)
	assert.Equal(t, "8.4.0", results[0].Version)
}

func TestParseChocoSearch_Empty(t *testing.T) {
	results := parseChocoSearch("")
...
func TestClient_Upgrade(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "choco", capturedName)
	assert.Equal(t, []string{"upgrade", "all", "-y"}, capturedArgs)
}
...
func TestClient_Upgrade_Specific(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "choco", capturedName)
	assert.Equal(t, []string{"upgrade", "-y", "curl", "wget"}, capturedArgs)
}
...
func TestClient_Remove(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "choco", capturedName)
	assert.Equal(t, []string{"uninstall", "-y", "curl", "wget"}, capturedArgs)
}
