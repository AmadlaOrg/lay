...
func TestBuilder_Build(t *testing.T) {
	...
	assert.Equal(t, "cargo", capturedName)
	assert.Equal(t, []string{"build", "--release"}, capturedArgs)
}
...
func TestBuilder_Build_Fails(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "cargo build failed")
}
