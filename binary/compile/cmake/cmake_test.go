...
func TestBuilder_Build(t *testing.T) {
	...
	assert.Equal(t, []string{"cmake", "cmake", "cmake"}, calls)
}
...
func TestBuilder_Build_ConfigureFails(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "cmake configure failed")
}

func TestBuilder_Build_BuildFails(t *testing.T) {
	origCmd := execCommand
	defer func() { execCommand = origCmd }()
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "cmake build failed")
}

func TestBuilder_Build_InstallFails(t *testing.T) {
	origCmd := execCommand
	defer func() { execCommand = origCmd }()
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "cmake install failed")
}
