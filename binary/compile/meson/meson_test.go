...
func TestBuilder_Build(t *testing.T) {
	...
	assert.Equal(t, []string{"meson", "ninja", "ninja"}, calls)
}

func TestBuilder_Build_SetupFails(t *testing.T) {
	origCmd := execCommand
	defer func() { execCommand = origCmd }()
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "meson setup failed")
}

func TestBuilder_Build_NinjaBuildFails(t *testing.T) {
	origCmd := execCommand
	defer func() { execCommand = origCmd }()
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "ninja build failed")
}

func TestBuilder_Build_NinjaInstallFails(t *testing.T) {
	origCmd := execCommand
	defer func() { execCommand = origCmd }()
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "ninja install failed")
}
